...
def convolve2d(img, kernel):
    #python task1.py --img_path data/proj1-task1.jpg --kernel prewitt
    ...
    for i, row in enumerate(img):
        for j, num in enumerate(row):
            img_patch = utils.crop(padded_img, i, i+3, j, j+3)
            element = utils.elementwise_mul(img_patch, flipped_kernel)
            img_conv[i][j] = element[0][0]+element[0][1]+element[0][2]+element[1][0]+element[1][1]+element[1][2]+element[2][0]+element[2][1]+element[2][2]
...
def edge_magnitude(edge_x, edge_y):
    ...
    for i, row in enumerate(edge_x):
        for j, num in enumerate(row):
            edge_mag[i][j] = np.sqrt((edge_x[i][j])**2.0 + (edge_y[i][j])**2.0)
